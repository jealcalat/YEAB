marginal_plot <- function(x, y,
  ...
  if (missing(group)) {
    if (missing(data)) {
      if (length(x) != length(y)) {
        stop("Length of arguments not equal")
      }
      data <- data.frame(x = as.numeric(x), y = as.numeric(y))
    } else {
      data <- data.frame(
        x = as.numeric(data[, deparse(substitute(x))]),
    ...
    if (sum(!complete.cases(data)) > 0) {
      warning(
        sprintf(
          "Removed %i rows with missing data",
  ...
  if (!is.null(moreargs$col)) {
    moreargs$col <- NULL
  }
  if (!is.null(moreargs$type)) {
    moreargs$type <- "p"
  }

  # get some default plotting arguments
  if (is.null(moreargs$xlim)) {
    moreargs$xlim <- range(data$x)
  }
  if (is.null(moreargs$ylim)) {
    moreargs$ylim <- range(data$y)
  }
  if (is.null(moreargs$xlab)) {
    moreargs$xlab <- deparse(substitute(x))
  }
  if (is.null(moreargs$ylab)) {
    moreargs$ylab <- deparse(substitute(y))
  }
  if (is.null(moreargs$las)) {
    moreargs$las <- 1
  }
  ...
  tryCatch(expr = {
    ...
    # upper density plot
    plot(NULL,
      type = "n", xlim = moreargs$xlim, ylab = "",
    ...
    if (missing(group)) {
      do.call(
        ...
        c(list(
          x = quote(data$x),
          y = quote(data$y),
          col = quote(scales::alpha("black", alpha))
        ), moreargs)
      )
    } else {
      do.call(
        ...
        c(list(
          x = quote(data$x),
          y = quote(data$y),
          col = quote(scales::alpha(group_colors[data$group], alpha))
        ), moreargs)
      )
    }
    ...
    plot(NULL,
      type = "n", ylim = moreargs$ylim,
      xlim = c(0, max(sapply(data_split, function(group_set) max(density(group_set$y, bw = bw)$y)))),
      main = NA, axes = F, xlab = ""
    )
    mapply(function(group_set, group_color) {
...
marginal_plot(x = Sepal.Width, y = Sepal.Length, group = Species, data = iris, lm_show = T, pch = 16)
